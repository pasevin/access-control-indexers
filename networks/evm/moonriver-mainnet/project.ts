...
/**
 * Moonriver Project Configuration
 * Chain ID: 1285
 */
import { MOONRIVER_MAINNET } from "@oz-indexers/network-config";

// Start block can be overridden via START_BLOCK env var (e.g., for staging deployments)
const startBlock =
  Number(process.env.START_BLOCK) || MOONRIVER_MAINNET.startBlock;

const project: EthereumProject = {
  specVersion: "1.0.0",
  version: "1.0.0",
  name: "oz-access-control-moonriver-mainnet",
  description: "OpenZeppelin Access Control and Ownable indexer for Moonriver",
  runner: {
    node: {
  ...
  network: {
    chainId: "1285",
    endpoint: [
      process.env.MOONRIVER_MAINNET_RPC_URL ||
